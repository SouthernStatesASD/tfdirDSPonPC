...
#ifndef ZEROMQ_ZHELPERS_HPP
#define ZEROMQ_ZHELPERS_HPP
...
static size_t
s_recv_binary (zmq::socket_t & socket, zmq::mutable_buffer *buffer) {

    zmq::detail::recv_buffer_result_t resultBuffer = socket.recv(*buffer, zmq::recv_flags::none);
    zmq::recv_buffer_size bufferSize = resultBuffer.value();
...
// custom send to send binary data
static bool
s_send_binary (zmq::socket_t & socket, const char *buffer, const int size) {

    zmq::const_buffer sendBuffer(buffer, size);
    zmq::detail::send_result_t sendResult = socket.send(sendBuffer, zmq::send_flags::none);

    if (size == sendResult.value())
        return true;
    else
        return false;
}
